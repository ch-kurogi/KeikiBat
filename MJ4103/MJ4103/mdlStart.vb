Module mdlStart
    ...
    ' 更新履歴：Rev002 - 2023.07.13 - EC H.Morii 高圧他テーブル対応
    ...
    Sub Main()
        ...
        Const wKINO_CD As String = "MJ4103"   '機能コード
...
#Region " 内部処理 "
    ...
    ' DB再接続
    '-------------------------------------------------------------------------
    Public Function ReConnectDB() As Boolean
        Dim cnt As Integer = 0
        Dim retval As Boolean = False

        '接続中チェック
        Try
            If myDB.CheckConnect() Then
                Return True
            End If
        Catch ex As Exception
            Return False
        ...
        If Not retval Then
            Call mdlStart.WriteApplicationLog("DB再接続処理に失敗しました。")
        End If
